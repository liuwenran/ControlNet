...
second_model.load_state_dict(load_state_dict('./models/control_sd15_normal.pth', location='cuda'))
...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold, init_latent, save_control_frame_ind):
    with torch.no_grad():
        ...
        ############# for normal map
        _, detected_map = apply_midas(resize_image(input_image, image_resolution), bg_th=0.4)
        detected_map = HWC3(detected_map)
        img = resize_image(input_image, image_resolution)
        H, W, C = img.shape
        ...
        second_control = torch.from_numpy(detected_map[:, :, ::-1].copy()).float().cuda() / 255.0
...
# a_prompt = 'best quality, masterpiece, best quality'
# n_prompt = 'longbody, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality'

prompt = 'a girl, smoking, black hair, goodlooking face, a man in the background, anime screencap, anime style'
a_prompt = 'best quality, masterpiece, extremely detailed, best quality'
n_prompt = 'longbody, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality'
...
save_folder = 'results/anythingv3_zhou_zenmela_canny_normal_sd15'
...
for ind in range(len(lines)):
    if use_init_noise:
        init_latent = init_noise_all_frame[1].unsqueeze(0)
