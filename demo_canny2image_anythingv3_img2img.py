...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, 
            scale, seed, eta, low_threshold, high_threshold, init_latent, save_control_frame_ind, last_control, save_folder):
    with torch.no_grad():
        ...
        if last_control is not None:
            momentum = 0.05
            control = momentum * control + (1 - momentum) * last_control
    ...
    return [255 - detected_map] + results, last_control
...
save_folder = 'results/anythingv3_zhou_zenmela_canny_debug_onlymidcancel'
...
for ind in range(len(lines)):
    ...
    results, last_control = process(img, prompt=prompt, a_prompt=a_prompt, n_prompt=n_prompt, num_samples=num_samples, image_resolution=image_resolution,
                  ...
                  seed=seed, eta=eta, init_latent=init_latent, save_control_frame_ind=save_control_frame_ind, last_control=last_control, save_folder=save_folder)
